...
#define AT_CMD_CONNECT_AP "AT+CWJAP"
static int wifi_start(hal_wifi_module_t *m, hal_wifi_init_type_t *init_para)
{
    ...
    snprintf(in, sizeof(in), "AT+CWMODE=3");
    ...
    if (strcmp(init_para->wifi_key, "open") == 0) {
        ...
        snprintf(in, sizeof(in), "AT+CWJAP=\"%s\",\"%s\"",
    ...
    if (strstr(out, AT_RSP_SUCCESS)) {
        LOGE(TAG, "AT command %s  ,  waiting for obtain IP ... \n", in);
    }
...
#define AT_CMD_OBTAIN_MAC "AT+CIFSR"
#define AT_RSP_MAC_PREFIX "+CIFSR:STAMAC,"
...
static int get_mac_helper(char *mac)
{
    ...
    if (at_send_wait_reply(at_dev_fd, at_mac_str, strlen(at_mac_str), true,
        ...
        LOGI(TAG, "AT command %s succeed, rsp: %s", AT_CMD_OBTAIN_MAC, "\nOK");
    } else {
        LOGD(TAG, "AT command %s failed\r\n", AT_CMD_OBTAIN_MAC);
        return -1;
    }

    if (strstr(out, AT_RSP_FAIL) || !strstr(out, AT_RSP_MAC_PREFIX)) {
        LOGD(TAG, "Command %s executed with ERROR.", AT_CMD_OBTAIN_MAC);
        return -1;
    }
    Mystr = strstr(out, AT_RSP_MAC_PREFIX);
    sscanf(Mystr, "%*[^:]:%[^\r]", mac);
...
#define AT_RSP_IP_PREFIX "+CIFSR:STAIP,"
static int get_ip_stat_helper(hal_wifi_ip_stat_t *result)
{
    char out[128] = {0};
    int ret = 0;
    ...
    if (strstr(out, AT_RSP_FAIL) || !strstr(out, AT_RSP_IP_PREFIX)) {
        LOGD(TAG, "Command  %s executed with ERROR", AT_CMD_OBTAIN_IP);
        return -1;
    }
     Mystr = strstr(out, AT_RSP_IP_PREFIX);
    sscanf(Mystr, "%*[^:]:%[^\r]", result->ip);
    LOGI(TAG, "result IP: %s \n",result->ip);
#if 0
    sscanf(out, "%*[^:]:%[^,],%[^,],%[^,],%[^\r]",
...
#define CURRENTAPINFO       "AT+CWJAP?"
#define CURRENTAPINFOPREFIX "+CWJAP"
...
hal_wifi_module_t aos_wifi_module_esp8266_12f = {
    .base.name           = "aos_wifi_module_esp8266_12f",
    .init                =  wifi_init,
