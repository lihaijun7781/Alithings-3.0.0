...
float alarm_setting =30.1;
bool alarm_clear = false;
void get_temp_kv(void)
{
    int res = 0;
    int len = 32;

    char buffer[32]={0};
    res = kv_item_get("alarm_setting", buffer, &len);
    if (res) {
        printf("can't get temp alert kv\r\n");
    } 
    else
    {
        float temp=-1;
...
float g_temp  = 0;
float g_humi  = 0;
void temp_humi_deal(void)
{
    static int overtmp = 0;
    float temp  = 0;
    float humi  = 0;
    int ret = 0;
    ...
    if (overtmp > 2)
    {
        overtmp = 3;
        if (alarm_clear)
            hal_gpio_output_low(&brd_gpio_table[GPIO_BEEP_PIN]);
        else
            hal_gpio_output_high(&brd_gpio_table[GPIO_BEEP_PIN]);
    }
    else
    {
        hal_gpio_output_low(&brd_gpio_table[GPIO_BEEP_PIN]);
