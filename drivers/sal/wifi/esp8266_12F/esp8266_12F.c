...
static void handle_socket_data()
{
    ...
    do
    {
        at_read(at_dev_fd, reader, 1);
        //LOGE(TAG,"%c",reader[0]);
        if (reader[0] == ',')
            break;
        link_id = link_id*10+ reader[0] -'0';
    }while(icnt++ < 5);
    memset(reader, 0, sizeof(reader));
    ...
    {
        aos_msleep(10);
        at_read(at_dev_fd, &recvdata[len-size], 100);
        size -= 100;
    }
    aos_msleep(5);
    ret = at_read(at_dev_fd, &recvdata[len-size], size);
...
static void handle_udp_broadcast_data()
{
    ...
    aos_msleep(100);
    at_read(at_dev_fd, &recvdata[len-size], size);
    recvdata[len] = '\0';
    ...
    LOGD(TAG, "###########get udp data  ##################\r\n");
    aos_free(recvdata);

}

static void esp8266_12f_get_local_ip_addr()
{
    int ret = 0;
...
static void esp8266_12f_wifi_event_handler(void *arg, char *buf, int buflen)
{
    char eventhead[4] = {0};
    char eventotal[16] = {0};
#if 0
    aos_sem_signal(&g_link[link_id].sem_start);

#endif 

    
    at_read(at_dev_fd, eventhead, 6);
    if (strcmp(eventhead,"GOT IP")==0)
	{	
        // GOT IP , send out message
        LOGE(TAG, "##GOT IP , send out message....  \n");
        //aos_sem_signal(&g_link[g_link_id].sem_start);
        aos_loop_schedule_call(NULL, esp8266_12f_get_local_ip_addr, NULL);
	}
    else if(strcmp(eventhead,"CONNEC")==0)
    {
        at_read(at_dev_fd, eventotal, 3);
        // wifi connected
    }
    else if(strcmp(eventhead,"DISCON")==0)
    {
        at_read(at_dev_fd, eventotal, 4);
...
// turn off AT echo
static void esp8266_12f_uart_echo_off()
{
...
#define NET_OOB_PREFIX "\r\n+IPD,"
#define WIFIEVENT_OOB_PREFIX "WIFI "
...
static int HAL_SAL_Init(void)
{
    ...
    esp8266_12f_uart_echo_off();

    snprintf(cmd, STOP_AUTOCONN_CMD_LEN-1, "AT+CIPMUX=1");
   at_send_wait_reply(at_dev_fd, cmd, strlen(cmd), true, NULL, 0, out, sizeof(out), NULL);
     
    LOGD(TAG, "The AT+CIPMUX response is: %s", out);
    ...
    for (link = 0; link < LINK_ID_MAX; link++) {
        ...
        LOGE(TAG, "%s %d - AT cmd to run: %s", __func__, __LINE__, cmd);

        at_send_wait_reply(at_dev_fd, cmd, strlen(cmd), true, NULL, 0, out, sizeof(out), NULL);
        LOGE(TAG, "The AT response is: %s", out);
        if (strstr(out, CMD_FAIL_RSP) != NULL) {
    ...
    at_register_callback(at_dev_fd, WIFIEVENT_OOB_PREFIX, NULL, NULL, 0, esp8266_12f_wifi_event_handler, NULL);
    inited = 1;
    //LOGE(TAG, "%s %d  #### xxinit EEEEd", __func__, __LINE__);
    return 0;
}
...
static char *start_cmd_type_str[] = {"TCP", "TCP", \
                                     "TCP", "UDP", "UDP"
                                    };
    char out[256] = {0};
static int HAL_SAL_Start(sal_conn_t *c)
{
    ...
    snprintf(cmd, START_CMD_LEN - 1, "AT+CIPCLOSE=%d",link_id);
   at_send_wait_reply(at_dev_fd, cmd, strlen(cmd), true, NULL, 0, out, sizeof(out), NULL);
     LOGD(TAG, "The AT close Link  response is: %s", out);
     LOGD(TAG, "Creating %s connection ...", start_cmd_type_str[c->type]);

    
    switch (c->type) {
        case TCP_SERVER:
        LOGD(TAG, "######Creating TCP server ##############\n");
        ...
        case UDP_BROADCAST:
        case UDP_UNICAST:
            snprintf(cmd, START_CMD_LEN - 2, "%s=%d,\"%s\",\"%s\",%d,%d,0",
                     START_CMD, link_id, start_cmd_type_str[c->type],
    ...
    LOGD(TAG, "\r\n%s %d - AT cmd to run: %s!  len:%d\r\n", __func__, __LINE__, cmd,strlen(cmd));
    ...
    if (aos_sem_wait(&g_link[link_id].sem_start, SEM_WAIT_DURATION) != 0) {
        LOGD(TAG, "%s sem_wait failed", __func__);
        //goto err;
    }
                 ...
                 int32_t timeout)
{
    ...
    //LOGE(TAG, "AT+CIPSEND link_id %d  remote_port%d  len:%d \n", __func__, remote_port,len);
    //LOGE(TAG, "Send: %s \n",data);
    /* AT+CIPSEND=id, */
                                 ...
                                 char ip[16])
{
    ...
    memcpy(ip, "116.211.167.65", strlen("116.211.167.65"));
...
#ifdef SAL_SERVER
static int esp8266_12f_client_status_notify(netconn_client_status_notify_t cb)
{
    if (cb) {
...
int esp8266_12f_sal_add_dev(char* driver_name, void* data)
{
...
static sal_op_t sal_op = {
    ...
    .version = "1.0.0",
    .name = "esp8266_12f",
    .add_dev = esp8266_12f_sal_add_dev,
    .init = HAL_SAL_Init,
    .start = HAL_SAL_Start,
...
int esp8266_12f_sal_device_init(void)
{
