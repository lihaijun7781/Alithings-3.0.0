...
#define ALARM_SETTING "alarm_setting"
...
const uint8_t *customsetting=0x0807c000;
static const char content[8][32]={{DEVINFO_DN},{DEVINFO_DS},{DEVINFO_PK},{DEVINFO_PS},{ALARM_SETTING},{NETMGR_WIFI_KEY},{DEVINFO_PK},{DEVINFO_PK}};
void init_triple_setting(void)
{
	int res = 0;
	int len = 128;
	char *str;
	...
	res = kv_item_get(DEVINFO_DN, buffer, &len);
	if (res) {
	puts("#########init  triple#########\n");
	int i = 0;
	for (i = 0;i < 7;i++)
	{
		str = &customsetting[128*i];
		...
		{
		case 0:
			HAL_SetDeviceName(buffer);
			break;
		case 1:
			HAL_SetDeviceSecret(buffer);
			break;
		case 2:
			HAL_SetProductKey(buffer);
			break;
		case 3:
			HAL_SetProductSecret(buffer);
			break;
		case 5:
			//netmgr_ap_config_t           saved_conf;
		       aos_kv_set(NETMGR_WIFI_KEY, buffer,
                     sizeof(netmgr_ap_config_t), 1);
			break;
		default:
			aos_kv_set(content[i],buffer,len,1);
			break;
		}
